  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        ...
        const userPrompt = `Here is the exam metadata:
- Exam ID: ${examId}
- Exam Name: ${examName}
- Course: ${course}
- Institution: ${institution}
- Year: ${year}
- Expected Total Score: ${scoreTotal || 'Unknown (calculate from questions)'}
- Default Tags: ${tags || 'None specified'}
...
Please generate the exam.md file following the exact format specified.`;
        ...
        log(`[4/5] Creating exam directory: ${examId}`);
        const examDir = path.join(COURSEEXAM_PATH, examId);
        ...
        log(`2. Run 'python prepare_dataset.py' in the courseexam_bench directory to regenerate the dataset`);
